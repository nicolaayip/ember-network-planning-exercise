...
import { forEachTxcFile, loadAdminAreas, loadCoachZip, readTxcEntry } from "../adapters/bods.js";
import { loadNaptan } from "../adapters/naptan.js";
import { parseTxcXml, parseTxc, expandJourneys, operatesOn, periodCovers } from "../domain/txc-parser.js";
import { flattenJourneys, dedupeJourneys, chainJourneys, pairKey } from "../domain/link-flattening.js";
...
export async function run(ctx) {
  ...
  if (archives.length === 0) {
    log.warn({ bods: sources.bods.listingSource, tnds: sources.tnds?.reason }, "supply skipped — no timetable source available; supplyVector left at defaults");
    resetSupplyVectors(doc);
    ctx.supply = { status: "unavailable", referenceDates, sources: describeSources(sources) };
    return ctx;
  }

  // 2. Scan pass: one TXC at a time → stop index + lightweight file refs (no bulk XML retention)
  const routeStops = Object.fromEntries(DIRECTIONS.map((d) => [d, doc.directions[d].orderedStops.filter(isPassenger)])); // depot: no competing services
  ...
  const { aliases, toleranceMatches } = toleranceAliases(routeStops, stopIndex, ourIds, settings.toleranceM);
  const candidateIds = new Set([...ourIds, ...Object.keys(aliases.outbound), ...Object.keys(aliases.return)]);
  ...
  const direct = allFiles.filter((f) => countInAtcos(f.atcos, candidateIds, pool.atcos) >= 1);
  const lineKeys = new Set(direct.flatMap((f) => [...f.lines].map((i) => `${f.source}|${pool.lines[i]}`)));
  const candidates = allFiles.filter((f) => direct.includes(f) || [...f.lines].some((i) => lineKeys.has(`${f.source}|${pool.lines[i]}`)));
  const coordsOf = (atco) => {
    const s = stopIndex.get(atco);
    return s?.lat !== undefined ? { lat: s.lat, lng: s.lng } : undefined;
  };
  log.info({ files: allFiles.length, candidateFiles: candidates.length, txcStops: stopIndex.size, toleranceAliases: toleranceMatches.length }, "supply: files scanned");
  ...
  for (const dayType of DAY_TYPES) {
    ...
    const chained = chainJourneys(deduped.journeys, { coordsOf, maxWaitMin: settings.chainMaxWaitMin, junctionRadiusM: settings.chainJunctionRadiusM });
    perDay[dayType] = {
      date,
    ...
    for (const d of DIRECTIONS) {
      ...
      perDay[dayType].byDirection[d] = flattenJourneys(chained.journeys, ids, { aliases: aliases[d] });
    }
  }
  ...
  for (const d of DIRECTIONS) {
    ...
    for (const pair of dir.directionalODPairs) {
      ...
      const wk = supplyDaySlice(perDay.weekday.byDirection[d].timelines.get(k) ?? [], tw, trafficProfile, d, "weekday", settings);
      const sat = supplyDaySlice(perDay.saturday.byDirection[d].timelines.get(k) ?? [], tw, trafficProfile, d, "saturday", settings);
      pair.supplyVector = {
...
const mapSchemaGaps = (gaps) =>
  gaps.map(({ gapStart, gapEnd, durationMinutes, isMarketOpening, peakOverlapMinutes }) => ({
    gapStart,
    gapEnd,
    durationMinutes,
    isMarketOpening,
...
async function loadTndsScotlandZip({ log, config }) {
  const file = config.supply?.tndsScotlandZip || path.join(config.paths.data, "tnds", "S.zip");
  try {
    await stat(file);
  } catch {
    return {
...
async function gatherSources(settings, log, config) {
  const sources = { bods: { listingSource: "none", perArea: {}, datasets: [] }, coach: null, tnds: null };
  try {
  ...
  try {
    ...
    if (sources.tnds.reason) log.warn({ reason: sources.tnds.reason }, "TNDS Scotland not loaded — Scottish-registered operators missing from supply");
  } catch (err) {
...
function txcArchives(sources) {
  const archives = sources.bods.datasets.filter((d) => d.file).map((d) => ({ path: d.file, source: `bods:${d.id}` }));
  if (sources.coach?.file) archives.push({ path: sources.coach.file, source: "coach" });
  if (sources.tnds?.file && !sources.tnds.reason) archives.push({ path: sources.tnds.file, source: "tnds" });
  return archives;
}

function describeSources(s, fileCounts = {}) {
  const bodsTotal = Object.entries(fileCounts).filter(([k]) => k.startsWith("bods:")).reduce((n, [, c]) => n + c, 0);
  return {
    bods: {
      listingSource: s.bods.listingSource,
      perAdminArea: s.bods.perArea,
      datasets: s.bods.datasets.map((d) => ({ id: d.id, operator: d.operatorName, noc: d.noc, lines: d.lines.length, adminAreas: d.adminAreas, files: fileCounts[`bods:${d.id}`] ?? 0, modified: d.modified, ...(d.error ? { error: d.error } : {}) })),
      files: bodsTotal,
    },
    coach: s.coach ? { url: "https://coach.bus-data.dft.gov.uk/TxC-2.4.zip", files: fileCounts.coach ?? 0 } : { files: 0, reason: "not available" },
    tnds: sourcesTndsOk(s) ? { file: s.tnds.file, files: fileCounts.tnds ?? 0 } : { files: 0, reason: s.tnds?.reason ?? "not available" },
  };
}
...
const RE_ANNOTATED = /<(?:\w+:)?AnnotatedStopPointRef\b[^>]*>([\s\S]*?)<\/(?:\w+:)?AnnotatedStopPointRef>/g;
const RE_STOPPOINT = /<(?:\w+:)?StopPoint\b[^>]*>([\s\S]*?)<\/(?:\w+:)?StopPoint>/g;
const RE_LINENAME = /<(?:\w+:)?LineName>\s*([^<]*?)\s*</g;
const tagText = (block, tag) => new RegExp(`<(?:\\w+:)?${tag}>\\s*([^<]*?)\\s*<`).exec(block)?.[1];
// Break V8 slice retention: interned strings must not reference parent TXC buffers.
const copy = (s) => (s ? Buffer.from(String(s), "utf8").toString("utf8").trim() : undefined);
...
function scanTxcXml(xml, index, pool) {
  ...
  const add = (atco, name, lat, lng) => {
    ...
    index.set(atco, { atco, name: name ?? cur?.name ?? "", ...(Number.isFinite(lat) && Number.isFinite(lng) ? { lat, lng } : {}) });
  };
  for (const m of xml.matchAll(RE_ANNOTATED)) {
    ...
    add(atco, copy(tagText(b, "CommonName")), Number(tagText(b, "Latitude")), Number(tagText(b, "Longitude")));
  }
...
async function fillMissingCoordinates(stopIndex, ctx, log) {
  ...
  try {
    ...
    for (const s of missing) {
      ...
      if (n) { s.lat = n.lat; s.lng = n.lng; if (!s.name) s.name = n.name; filled++; }
    }
    log.debug({ missing: missing.length, filled }, "supply: TXC stop coordinates from NaPTAN");
  } catch (err) {
    log.warn({ missing: missing.length, reason: err.message }, "supply: NaPTAN unavailable for TXC stops without coordinates");
  }
}
...
function toleranceAliases(routeStops, stopIndex, ourIds, toleranceM) {
  ...
  const matches = [];
  const located = [...stopIndex.values()].filter((s) => s.lat !== undefined && !ourIds.has(s.atco));
  for (const d of DIRECTIONS) {
    ...
    routeStops[d].forEach((rs, i) => {
      for (const s of located) {
        ...
        if (!cur || dm < cur.distanceM) best.set(s.atco, { index: i, distanceM: Math.round(dm), routeStopId: rs.naptanId, routeStopName: rs.stopName, txcName: s.name });
      }
    });
    for (const [atco, b] of best) {
      aliases[d][atco] = b.index;
      matches.push({ direction: d, routeStopId: b.routeStopId, routeStopName: b.routeStopName, txcStopId: atco, txcStopName: b.txcName, distanceM: b.distanceM });
    }
  }
...
async function parseCandidates(files, log) {
  ...
  for (const f of files) {
    try {
      ...
      failures++;
      log.warn({ file: f.name, source: f.source, reason: err.message }, "supply: TXC parse failed");
    }
  }
...
function selectJourneys(journeys, date, includeSchoolDays) {
  const out = [];
  let schoolDayOnlyExcluded = 0, calendarAssumed = 0;
  for (const j of journeys) {
    ...
    if (r.schoolDaysOnly && !includeSchoolDays) { schoolDayOnlyExcluded++; continue; }
    out.push(j);
  }
...
function retainedServices(perDay, routeStops) {
  ...
  for (const d of DIRECTIONS) {
    for (const t of DAY_TYPES) {
      for (const [key, svc] of perDay[t].byDirection[d].services) {
        const k = `${d}|${key}`;
        const rec = byKey.get(k) ?? { operator: svc.operator, noc: svc.noc, line: svc.line, txcDirection: svc.direction, routeDirection: d, journeysPerDay: { weekday: 0, saturday: 0, sunday: 0 }, sharedStops: new Set() };
  ...
  return [...byKey.values()]
    .map((r) => ({ ...r, sharedStops: [...r.sharedStops].sort((a, b) => a - b).map((i) => routeStops[r.routeDirection][i].stopName) }))
    .sort((a, b) => a.routeDirection.localeCompare(b.routeDirection) || b.journeysPerDay.weekday - a.journeysPerDay.weekday);
}
...
function coverageGaps(routeStops, sources) {
  const covered = new Set(sources.bods.datasets.filter((d) => d.file).flatMap((d) => d.adminAreas));
  ...
  for (const d of DIRECTIONS) {
    for (const s of routeStops[d]) {
      ...
      if (!covered.has(area) && !(scottish && tndsLoaded)) {
        incomplete.push({ direction: d, naptanId: s.naptanId, stopName: s.stopName, adminArea: area, reason: scottish ? "no BODS dataset for this admin area; TNDS Scotland not loaded" : "no BODS dataset for this admin area" });
      }
    }
  }
  return { coveredAdminAreas: [...covered].sort(), tndsLoaded, incompleteStops: incomplete };
}

function resetSupplyVectors(doc) {
  for (const d of DIRECTIONS) for (const p of doc.directions[d].directionalODPairs) p.supplyVector = emptySupplyVector();
}
