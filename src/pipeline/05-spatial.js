/**
 * Pipeline step 5 — SPATIAL RESOLUTION.
 *
...
export async function run(ctx) {
  try {
    ...
    if (
      ...
      /OPENROUTESERVICE_API_KEY is not set/.test(err.message)
    ) {
      ctx.log.warn(
        { reason: err.message },
        "spatial skipped — OpenRouteService unavailable",
      );
      return ctx;
    }
    throw err;
...
async function compute(ctx) {
  ...
  for (const d of DIRECTIONS) {
    for (const stop of doc.directions[d].orderedStops) {
      ...
      if (ctx.catchments[stop.naptanId]) {
        stop.catchment = ctx.catchments[stop.naptanId];
        continue;
      }

      // ORS (throttled only when the call is not cached)
      const wait = ORS_MIN_INTERVAL_MS - (Date.now() - lastOrsCall);
      const iso = await isochrone(stop.coordinates, seconds, { log }).catch(
        async (err) => {
          if (wait > 0 && /429/.test(err.message)) {
            await sleep(wait);
            return isochrone(stop.coordinates, seconds, { log });
          }
          throw err;
        },
      );
      if (!iso.cached) {
        ...
        await sleep(ORS_MIN_INTERVAL_MS);
      }

      // Boundaries + population
      const zones = await zonesIntersecting(iso.polygon.geometry, log);
      for (const z of zones)
        if (z.country === "england-wales") z.population = lsoaPop.get(z.code) ?? null;
      boundaryZones += zones.length;

      const c = proportionalCatchment(iso.polygon, zones);
      if (c.unpopulatedZones.length)
        log.warn(
          { stop: stop.stopName, zones: c.unpopulatedZones },
          "zones without population",
        );
      ...
      stop.catchment = {
        ...
        zones: c.zones.map(({ code, name, zonePopulation, ratio, allocated }) => ({
          code,
          name,
          zonePopulation,
          ratio,
  ...
  log.info(
    { stops: Object.keys(ctx.catchments).length, orsCalls, boundaryZones },
    "catchments computed",
  );
  return ctx;
}
